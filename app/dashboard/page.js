...
function CategoriesPage({ company, C, isMobile }) {
  ...
  const loadCats = async () => {
    const { data } = await supabase.from('categories').select('*').eq('company_id', company.id).order('order_index')
    const all = data || []
    // Para birimi kategorileri yoksa otomatik oluştur
    const hasCurrency = all.some(c => c.type === 'currency')
    if (!hasCurrency) {
      const currencies = DEFAULT_CURRENCY_CODES.map((code, i) => ({
        company_id: company.id, name: code, type: 'currency',
  ...
  const openNew = () => {
    setEditItem(null)
    setForm({ name: '', type: filter === 'currency' ? 'currency' : filter === 'income' ? 'income' : 'expense', color: '#888780' })
    setErrors({})
    setModal(true)
  }
  ...
  const save = async () => {
    const errs = validate({
      name: { required: true, message: 'Kategori adı zorunludur' },
    }, form)
    if (Object.keys(errs).length > 0) { setErrors(errs); return }
    setSaving(true)
    const payload = { company_id: company.id, name: form.name, type: form.type, color: form.color }
    if (editItem) {
      const { data } = await supabase.from('categories').update(payload).eq('id', editItem.id).select().single()
      if (data) setCats(cats.map(c => c.id === editItem.id ? data : c))
    } else {
  ...
  const handleDelete = async (cat) => {
    ...
    const { count } = await supabase.from('transactions').select('id', { count: 'exact', head: true }).eq('category_id', cat.id)
    if (count && count > 0) {
      setDeleteError(`"${cat.name}" kategorisi ${count} işlemde kullanılıyor. Silinemez, pasif yapabilirsiniz.`)
      return
    }
    await supabase.from('categories').delete().eq('id', cat.id)
    setCats(cats.filter(c => c.id !== cat.id))
  ...
  const currencyMeta = (code) => CURRENCY_META[code] || { symbol: code, label: code }
  ...
  return (
    <div>
      ...
      {deleteError && (
        <div style={{ background: C.redBg, border: `1px solid ${C.red}`, borderRadius: '8px', padding: '12px 14px', marginBottom: '12px', fontSize: '13px', color: C.red }}>
          ⚠ {deleteError}
          <button onClick={() => setDeleteError('')} style={{ float: 'right', background: 'none', border: 'none', cursor: 'pointer', color: C.red, fontSize: '16px' }}>×</button>
        </div>
      )}
        ...
        filtered.length === 0 ? <div style={{ padding: '40px', textAlign: 'center', color: C.text3, fontSize: '13px' }}>Kategori bulunamadı</div> : (
          <ScrollTable minWidth="400px">
            ...
            <tbody>
              {filtered.map((c, i) => {
                const isCurrency = c.type === 'currency'
                const meta = isCurrency ? currencyMeta(c.name) : null
                return (
                  <tr key={c.id} style={{ borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}` : 'none', opacity: c.is_active ? 1 : 0.45 }}
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        {isCurrency ? (
                          <div style={{ width: '26px', height: '26px', borderRadius: '6px', background: c.color || C.amberBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: '800', color: '#fff' }}>{meta?.symbol}</div>
                        ) : (
                          <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: c.color || '#888', flexShrink: 0 }}/>
                        )}
                        <div>
                          <div style={{ fontWeight: '600', color: C.dark }}>{isCurrency ? (meta?.label || c.name) : c.name}</div>
                          {isCurrency && <div style={{ fontSize: '11px', color: C.text3 }}>Kod: {c.name}</div>}
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{ fontSize: '11px', fontWeight: '600', padding: '2px 8px', borderRadius: '20px',
                        background: isCurrency ? 'rgba(27,46,94,0.08)' : c.type==='income'?C.greenBg:C.redBg,
                        color: isCurrency ? C.dark : c.type==='income'?C.green:C.red }}>
                        {isCurrency ? '💱 Para Birimi' : c.type==='income'?'Gelir':'Gider'}
                      </span>
                    </td>
                    <td style={{ padding: '10px 14px' }}>
      ...
      {modal && (
        <Modal title={editItem ? 'Kategoriyi Düzenle' : 'Yeni Kategori'} onClose={() => setModal(false)} onSave={save} saving={saving} C={C}>
          <FField label="Kategori Adı" required C={C} error={errors.name}>
            <input value={form.name} onChange={e => { setForm({...form, name: e.target.value}); setErrors({...errors, name: ''}) }}
              placeholder={form.type === 'currency' ? 'TRY, USD, EUR, GOLD...' : 'Malzeme, İşçilik...'}
              style={inp(C, errors.name)}/>
          </FField>
          <FField label="Tür" C={C}>
            <select value={form.type} onChange={e => setForm({...form, type: e.target.value})} style={inp(C)}>
...
function ProjectsPage({ company, fmt, fmtDate, C, isMobile }) {
  ...
  const [editItem, setEditItem] = useState(null)
  const [filter, setFilter] = useState('all')
  const emptyForm = { name: '', address: '', budget: 0, start_date: '', end_date: '', status: 'active', description: '' }
  const [form, setForm] = useState(emptyForm)
  const [errors, setErrors] = useState({})
  const [saving, setSaving] = useState(false)
  ...
  const openNew = () => { setEditItem(null); setForm(emptyForm); setErrors({}); setModal(true) }
  const openEdit = (p, e) => { e?.stopPropagation(); setEditItem(p); setForm({ name: p.name, address: p.address||'', budget: p.budget||0, start_date: p.start_date||'', end_date: p.end_date||'', status: p.status, description: p.description||'' }); setErrors({}); setModal(true) }
  ...
  return (
    <div>
      ...
      {modal && (
        <Modal title={editItem ? 'Projeyi Düzenle' : 'Yeni Proje'} onClose={() => setModal(false)} onSave={save} saving={saving} C={C}>
          <FField label="Proje Adı" required C={C} error={errors.name}>
            <input value={form.name} onChange={e => { setForm({...form, name: e.target.value}); setErrors({...errors, name: ''}) }} placeholder="Kadıköy Konut Projesi" style={inp(C, errors.name)}/>
          </FField>
          <FRow>
